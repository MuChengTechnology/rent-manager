...
每次模擬考會保存 attempt ID、交卷時間、題庫類型、總分、十章的總題數／作答數／答對數，以及該次答錯或未作答題目的穩定 key、canonical 作答選項代號（未作答為 null）與單題內容指紋。歷史回顧只會在目前題庫／註記內容與指紋一致，且作答資料沒有矛盾時，使用目前 canonical 題目推導題目、作答選項、正確選項與說明；本機紀錄永遠不決定正確答案，也不重建當次隨機排列的畫面代號。不保存完整題目文字、四個選項內容或整份題序，舊紀錄仍可查看章節統計但不會虛構逐題作答，題庫內容更新時也不會混用新題目與舊作答。最多保留最近 50 次結果。`/mock` 的「清除模擬考紀錄」只會清除結果分布與該次錯題回顧，不影響錯題與累計作答；`/wrong` 的「重設本機紀錄」則會清除全部本機學習紀錄。也可在瀏覽器開發者工具的 **Application／儲存空間** 清除網站資料。
